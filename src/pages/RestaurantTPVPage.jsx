  ...
  return (
    <div className="flex gap-6 h-[calc(100vh-8rem)]">
      ...
      <div className="w-1/2 flex flex-col">
        {/* Tabs */}
        <div className="flex items-center gap-1 p-1 mb-4 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200/50 dark:border-white/5 w-fit">
          <button onClick={() => { setActiveTab('tables'); if (selectedTable?.isDelivery) selectTable(null); }}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'tables' ? 'bg-white dark:bg-slate-800 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
            <UtensilsCrossed size={16} /> Mesas
          </button>
          <button onClick={() => { setActiveTab('delivery'); if (selectedTable && !selectedTable.isDelivery) selectTable(null); }}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'delivery' ? 'bg-white dark:bg-slate-800 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
        ...
        {activeTab === 'tables' && (
          <>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Mesas</h2>
            ...
            {showAddTable && (
              <div className="mb-4 p-4 rounded-2xl border border-primary-200/50 dark:border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur-xl space-y-3">
              ...
              {localTables.map((table) => {
                ...
                return (
                  <div key={table.id} onClick={() => handleSelectTable(table)}
                    className={`relative p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 hover:scale-[1.02] group
                    ...
                    <button onClick={(e) => { e.stopPropagation(); handleDeleteTable(table.id); }}
                      className="absolute top-2 right-2 p-1 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-500/20 text-red-500 transition-all">
                      <Trash2 size={14} />
                    </button>
                    {itemCount > 0 && (
                      <div className="absolute -top-2 -left-2 w-6 h-6 rounded-full bg-amber-500 text-white text-xs font-bold flex items-center justify-center shadow-lg">{itemCount}</div>
                    )}
                    <div className="text-center">
                      <p className="font-bold text-lg">{table.name}</p>
                      <div className="flex items-center justify-center gap-1 mt-1 text-xs opacity-70"><Users size={12} /><span>{table.capacity}</span></div>
                      <span className="inline-block mt-2 text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-current/10">
        ...
        {activeTab === 'delivery' && (
          <>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Domicilios</h2>
              <button onClick={() => setShowAddDelivery(!showAddDelivery)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-primary-500 hover:bg-primary-600 text-white text-sm font-medium transition-colors">
                <Plus size={16} /> Nuevo Domicilio
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 overflow-y-auto flex-1 content-start">
              {deliveryOrders.map((order) => {
                ...
                return (
                  <div key={order.id} onClick={() => handleSelectTable(order)}
                    ...
                    <div className="text-center">
                      <Bike size={22} className="mx-auto mb-1 text-primary-600 dark:text-primary-400" />
                      <p className="font-bold text-sm truncate">{order.name}</p>
                      <span className="inline-block mt-1 text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-current/10">
              ...
              {deliveryOrders.length === 0 && (
                <div className="col-span-3 text-center py-12 text-slate-400">
                  ...
                  <p className="font-medium">No hay domicilios activos</p>
                  <p className="text-sm mt-1">Crea un nuevo domicilio para comenzar</p>
                </div>
              )}
            </div>
          </>
      ...
      {/* ═══ Right Panel - Order ═══ */}
      <div className="w-1/2 flex flex-col rounded-2xl border border-white dark:border-white/5 bg-white/80 dark:bg-slate-900/40 backdrop-blur-2xl shadow-xl overflow-hidden">
        {selectedTable ? (
          <>
            {/* Header */}
            <div className="px-5 py-4 border-b border-slate-200/50 dark:border-white/5 bg-primary-50/50 dark:bg-primary-900/10">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-lg text-slate-800 dark:text-white">
                    {selectedTable.isDelivery ? selectedTable.name : selectedTable.name}
                  </h3>
                  <p className="text-xs text-slate-500">
                    {selectedTable.isDelivery ? (
                      <>Pedido a domicilio</>
                    ) : (
                      <>{TABLE_STATUS_LABELS[selectedTable.status]} · {selectedTable.capacity} personas</>
                    )}
                    {hasPendingSale && <span className="ml-2 text-amber-500 font-semibold">· Orden activa</span>}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {(cart.length > 0 || hasPendingSale) && (
                    <button onClick={() => setShowCancelConfirm(true)}
                      className="p-2 rounded-xl hover:bg-red-100 dark:hover:bg-red-900/20 text-red-500 transition-colors" title="Cancelar orden">
                      <XCircle size={18} />
                    </button>
                  )}
                  <button onClick={() => selectTable(null)}
                    className="p-2 rounded-xl hover:bg-slate-200/50 dark:hover:bg-white/5 text-slate-500 transition-colors">
                    <X size={18} />
                  </button>
                </div>
              </div>
            ...
            <div className="px-5 py-3 border-b border-slate-200/50 dark:border-white/5">
              <div className="relative">
                ...
                <input type="text" placeholder="Buscar producto..." value={searchQuery} onChange={handleSearch}
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 bg-white/70 dark:bg-white/5 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 focus:border-primary-500/50 focus:outline-none transition-colors text-sm" />
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200/50 dark:border-white/5">
                    {editingItem === item.id ? (
                      ...
                      <>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{item.customName || item.name}</p>
                          <p className="text-xs text-primary-600 dark:text-primary-400 font-semibold">${Number(item.salePrice).toLocaleString()} c/u</p>
                        </div>
                        <button onClick={() => handleStartEdit(item)} className="p-1.5 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-900/30 text-primary-500 transition-colors"><Edit3 size={14} /></button>
                        <div className="flex items-center gap-1.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-white/10 px-1">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg transition-colors text-slate-600 dark:text-slate-400"><Minus size={14} /></button>
                          <span className="w-8 text-center text-sm font-bold text-slate-800 dark:text-white">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg transition-colors text-slate-600 dark:text-slate-400"><Plus size={14} /></button>
                        </div>
                        <p className="text-sm font-bold text-slate-800 dark:text-white w-20 text-right">${(Number(item.salePrice) * item.quantity).toLocaleString()}</p>
                        <button onClick={() => removeFromCart(item.id)} className="p-1.5 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 text-red-500 transition-colors"><Trash2 size={14} /></button>
                      </>
                    )}
                  </div>
                ))
            ...
            {/* Footer */}
            <div className="border-t border-slate-200/50 dark:border-white/5 px-5 py-4 space-y-3 bg-slate-50/50 dark:bg-white/[0.02]">
              <div className="space-y-1">
                <div className="flex justify-between text-sm text-slate-500"><span>Subtotal</span><span>${subtotal.toLocaleString()}</span></div>
                <div className="flex justify-between text-sm text-slate-500"><span>IVA (19%)</span><span>${tax.toLocaleString()}</span></div>
                <div className="flex justify-between text-lg font-bold text-slate-800 dark:text-white pt-1 border-t border-slate-200 dark:border-white/10"><span>Total</span><span>${total.toLocaleString()}</span></div>
              </div>
              {processingMessage && (
                <div className="text-center py-2 px-4 rounded-xl bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-sm font-medium">{processingMessage}</div>
              )}
              <div className="flex gap-2">
                <button onClick={handleComanda} disabled={cart.length === 0 || loading}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                  <Printer size={18} /> Comanda
                </button>
                <button onClick={handleCobrar} disabled={cart.length === 0 || loading}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-primary-600 hover:bg-primary-500 text-white font-medium shadow-lg shadow-primary-600/30 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          <div className="flex-1 flex items-center justify-center text-slate-400">
            <div className="text-center">
              {activeTab === 'tables' ? (
                <><UtensilsCrossed size={56} className="mx-auto mb-4 opacity-20" /><p className="font-medium text-lg">Selecciona una mesa</p><p className="text-sm mt-1">Elige una mesa del panel izquierdo</p></>
              ) : (
                <><Bike size={56} className="mx-auto mb-4 opacity-20" /><p className="font-medium text-lg">Selecciona un domicilio</p><p className="text-sm mt-1">Crea o elige un domicilio del panel izquierdo</p></>
              )}
            </div>
          </div>
        )}
