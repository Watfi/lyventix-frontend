...
const SuppliersPage = () => {
  ...
  const handleDelete = async (id) => { if (!confirm('¿Eliminar este proveedor?')) return; try { await supplierService.deleteSupplier(id); fetchData(); } catch (err) { setError(err.response?.data?.message || 'Error'); } };

  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div><h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Proveedores</h2><p className="text-slate-500 dark:text-slate-400 mt-1">Gestiona tus proveedores y condiciones de pago</p></div>
        <Button onClick={() => { setEditing(null); setForm({ name: '', taxId: '', email: '', phone: '', address: '', contactName: '', paymentTerms: '' }); setShowModal(true); }}><Plus size={20} /><span>Nuevo Proveedor</span></Button>
      </div>

      <div className="glass-panel p-4 rounded-3xl"><Input className="flex-1" placeholder="Buscar proveedores..." icon={Search} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
      ...
      {loading ? <div className="flex items-center justify-center h-40"><Loader2 className="animate-spin text-primary-400" size={40} /></div> : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {suppliers.map((s) => (
            <motion.div key={s.id} whileHover={{ y: -4 }} className="glass-card p-6 rounded-3xl relative group">
              ...
              <div className="mt-6 pt-4 border-t border-slate-200 dark:border-white/5 flex items-center justify-between">
                <span className={`px-2 py-1 rounded-lg text-xs font-semibold ${s.active !== false ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>{s.active !== false ? 'Activo' : 'Inactivo'}</span>
          ...
          {suppliers.length === 0 && <div className="col-span-full text-center py-16 text-slate-500"><Truck size={48} className="mx-auto mb-4 opacity-40" /><p>No hay proveedores</p></div>}
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-panel rounded-3xl p-8 w-full max-w-lg">
            <div className="flex items-center justify-between mb-6"><h3 className="text-xl font-bold text-slate-800 dark:text-white">{editing ? 'Editar Proveedor' : 'Nuevo Proveedor'}</h3><button onClick={() => setShowModal(false)} className="text-slate-500 dark:text-slate-400 hover:text-white"><X size={20} /></button></div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <Input label="Nombre *" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
              <div className="grid grid-cols-2 gap-4">
                <Input label="NIT/RUT" value={form.taxId} onChange={(e) => setForm({ ...form, taxId: e.target.value })} />
              ...
              <Input label="Condiciones de Pago" value={form.paymentTerms} onChange={(e) => setForm({ ...form, paymentTerms: e.target.value })} placeholder="Ej: 30 días, contado..." />
