  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div><h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">{t('categories_title')}</h2><p className="text-slate-500 dark:text-slate-400 mt-1">{t('categories_desc')}</p></div>
        <Button onClick={() => { setEditing(null); setForm({ name: '', description: '', code: '', color: '#3b82f6', icon: '', displayOrder: '' }); setShowModal(true); }}><Plus size={20} /><span>{t('categories_btn_new')}</span></Button>
      </div>
      ...
      {loading ? <div className="flex items-center justify-center h-40"><Loader2 className="animate-spin text-primary-400" size={40} /></div> : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {categories.map((c) => (
            <motion.div key={c.id} whileHover={{ y: -4 }} className="glass-card p-6 rounded-3xl relative group">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-white/5">
                <span className={`px-2 py-1 rounded-lg text-xs font-semibold ${c.active !== false ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>{c.active !== false ? t('active') : t('inactive')}</span>
          ...
          {categories.length === 0 && <div className="col-span-full text-center py-16 text-slate-500"><Tag size={48} className="mx-auto mb-4 opacity-40" /><p>{t('categories_no_found')}</p></div>}
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-panel rounded-3xl p-8 w-full max-w-lg">
            <div className="flex items-center justify-between mb-6"><h3 className="text-xl font-bold text-slate-800 dark:text-white">{editing ? t('categories_modal_edit_title') : t('categories_modal_new_title')}</h3><button onClick={() => setShowModal(false)} className="text-slate-500 dark:text-slate-400 hover:text-white"><X size={20} /></button></div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <Input label={t('categories_form_name')} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
              <div className="grid grid-cols-2 gap-4">
                <Input label={t('categories_form_code')} value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} />
                <div><label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">{t('categories_form_color')}</label><input type="color" value={form.color} onChange={(e) => setForm({ ...form, color: e.target.value })} className="w-full h-12 rounded-xl border border-slate-200 dark:border-white/10 bg-transparent cursor-pointer" /></div>
              </div>
              <div><label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">{t('description')}</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-800 dark:text-white outline-none focus:border-primary-500 h-20 resize-none" /></div>
