...
const BranchesPage = () => {
  ...
  const handleDelete = async (id) => { if (!confirm('¿Eliminar esta sucursal?')) return; try { await branchService.deleteBranch(businessId, id); fetch(); } catch (err) { setError(err.response?.data?.message || 'Error'); } };

  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div><h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Sucursales</h2><p className="text-slate-500 dark:text-slate-400 mt-1">Administra las sedes de tu negocio</p></div>
        <Button onClick={() => { setEditing(null); setForm({ code: '', name: '', phone: '', email: '', address: '', city: '', state: '', postalCode: '', isMain: false }); setShowModal(true); }}><Plus size={20} /><span>Nueva Sucursal</span></Button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {branches.map((b) => (
            <motion.div key={b.id} whileHover={{ y: -4 }} className="glass-card p-6 rounded-3xl relative group">
              {b.isMain && <div className="absolute top-4 right-4 flex items-center gap-1 bg-amber-500/10 text-amber-400 text-xs font-bold px-2 py-1 rounded-lg"><Star size={12} /> Principal</div>}
              ...
              <div className="mt-6 pt-4 border-t border-slate-200 dark:border-white/5 flex items-center justify-between">
                <span className={`px-2 py-1 rounded-lg text-xs font-semibold ${b.active !== false ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>{b.active !== false ? 'Activa' : 'Inactiva'}</span>
          ...
          {branches.length === 0 && <div className="col-span-full text-center py-16 text-slate-500"><Building2 size={48} className="mx-auto mb-4 opacity-40" /><p>No hay sucursales</p></div>}
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-panel rounded-3xl p-8 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6"><h3 className="text-xl font-bold text-slate-800 dark:text-white">{editing ? 'Editar Sucursal' : 'Nueva Sucursal'}</h3><button onClick={() => setShowModal(false)} className="text-slate-500 dark:text-slate-400 hover:text-white"><X size={20} /></button></div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={form.isMain} onChange={(e) => setForm({ ...form, isMain: e.target.checked })} className="w-4 h-4 rounded border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/5 text-primary-600" /><span className="text-slate-600 dark:text-slate-300 text-sm">Sucursal principal</span></label>
